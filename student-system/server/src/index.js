...
app.get("/subjects/:subjectId/students", authRequired, requireRole("subjects"), async (req, res) => {
  ...
  try {
    ...
    if (role === "teacher") {
      const teacher = await get("SELECT full_name FROM users WHERE id=?", [id]);
      const fullName = teacher?.full_name || username;
      const isOwner = await get("SELECT 1 FROM subjects WHERE id=? AND (professor=? OR professor=?)", [subjectId, username, fullName]);
      if (!isOwner) return res.status(403).json({ error: "Forbidden: Not your subject" });
    }
...
app.get("/dashboard-stats", async (req, res) => {
  ...
  if (role === "teacher") {
    ...
    const mySubs = await get("SELECT COUNT(*) as c FROM subjects WHERE (professor=? OR professor=?) AND deleted_at IS NULL", [username, fullName]);
    stats.activeSubjects = parseInt(mySubs?.c || 0);

    const myGrades = await get(`
      SELECT COUNT(*) as c FROM grades g
      JOIN subjects b ON b.id = g.subject_id AND b.deleted_at IS NULL
      JOIN students s ON s.id = g.student_id AND s.deleted_at IS NULL
      WHERE g.deleted_at IS NULL AND (b.professor=? OR b.professor=?)
    `, [username, fullName]);
    stats.gradeRecords = parseInt(myGrades?.c || 0);

    const myStudents = await get(`
      SELECT COUNT(DISTINCT g.student_id) as c FROM grades g
      JOIN subjects b ON b.id = g.subject_id AND b.deleted_at IS NULL
      JOIN students s ON s.id = g.student_id AND s.deleted_at IS NULL
      WHERE g.deleted_at IS NULL AND (b.professor=? OR b.professor=?)
    `, [username, fullName]);
    stats.totalStudents = parseInt(myStudents?.c || 0);
    ...
    const atRisk = await all(`
      WITH subject_avgs AS (
        SELECT 
          g.student_id,
          g.subject_id,
          ( (COALESCE(g.prelim1,0) + COALESCE(g.prelim2,0) + COALESCE(g.midterm,0) + COALESCE(g.semi_final,0) + COALESCE(g.final,0))::FLOAT / 
            NULLIF((CASE WHEN g.prelim1 IS NULL THEN 0 ELSE 1 END) + 
                   (CASE WHEN g.prelim2 IS NULL THEN 0 ELSE 1 END) + 
                   (CASE WHEN g.midterm IS NULL THEN 0 ELSE 1 END) + 
                   (CASE WHEN g.semi_final IS NULL THEN 0 ELSE 1 END) + 
                   (CASE WHEN g.final IS NULL THEN 0 ELSE 1 END), 0)
          ) as subj_avg
        FROM grades g
        JOIN subjects b ON b.id = g.subject_id AND b.deleted_at IS NULL
        WHERE g.deleted_at IS NULL
          AND (b.professor=? OR b.professor=?)
          AND (g.prelim1 IS NOT NULL OR g.prelim2 IS NOT NULL OR g.midterm IS NOT NULL OR g.semi_final IS NOT NULL OR g.final IS NOT NULL)
      )
  ...
  // Staff Dashboard (Full access for Owner/Registrar/Developer)
...
app.delete(
  ...
  async (req, res) => {
    ...
    await tx(async () => {
      ...
      // NEW: Clear grade change requests for the deleted student
      await run("DELETE FROM grade_change_requests WHERE student_id=?", [id]);
...
app.get("/subjects", authRequired, requireRole("subjects"), async (req, res) => {
  ...
  if (role === "teacher") {
    ...
    const teacher = await get("SELECT full_name FROM users WHERE id=?", [userId]);
    const fullName = teacher?.full_name || username;
    const rows = await all("SELECT * FROM subjects WHERE (professor=? OR professor=?) AND deleted_at IS NULL", [username, fullName]);
    return res.json(rows);
  }
  if (role === "student") {
  ...
  if (role === "teacher") {
    ...
    const rows = await all(
      `SELECT g.* FROM grades g
       JOIN students s ON s.id = g.student_id AND s.deleted_at IS NULL
       JOIN subjects b ON b.id = g.subject_id AND b.deleted_at IS NULL
       WHERE g.deleted_at IS NULL AND (b.professor=? OR b.professor=?)` + (semesterId ? ` AND g.semester_id = ?` : ``),
      semesterId ? [username, fullName, semesterId] : [username, fullName]
    );
    return res.json(rows);
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      const activeSubject = await get(
        "SELECT professor FROM subjects WHERE id=? AND deleted_at IS NULL",
      ...
      if (req.user.role === "teacher") {
        ...
        const teacher = await get("SELECT full_name FROM users WHERE id=?", [userId]);
        const fullName = teacher?.full_name || username;
        if (activeSubject.professor !== username && activeSubject.professor !== fullName) {
  ...
  try {
    ...
    if (role === "teacher") {
      rows = await all(`
        ...
        JOIN subjects sub ON sub.id = g.subject_id
        WHERE g.teacher_username = ? AND s.deleted_at IS NULL
